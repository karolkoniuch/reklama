...
class Process
{
   ...
   function procRegister(){
      ...
      $retval = $session->register($_POST['user'], $_POST['pass'], $_POST['conf_pass'], $_POST['email'], $_POST['conf_email'], $_POST['ref']);
      }
   ...
   function procForgotPass(){
      ...
      if(!$subuser || strlen($subuser = trim($subuser)) == 0){
         $form->setError($field, "* Wpisz login<br>");
      }
      else{
         ...
         if(strlen($subuser) < $config['min_user_chars'] || strlen($subuser) > $config['max_user_chars'] ||    
            ...
            $form->setError($field, "* Nie ma takiego loginu<br>");
          } 
          else if ($database->checkUserEmailMatch($subuser, $subemail) == 0){
          	$form->setError($field, "* E-Mail nie pasuje<br>");
       }
      }
